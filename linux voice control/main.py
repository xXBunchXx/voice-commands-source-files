...
def _do_update(root: tk.Tk, status_var: tk.StringVar, latest_version: str) -> None:
    """Download the new Linux binary and replace the running executable."""
    bin_url  = f"{GITHUB_RELEASES}/v{latest_version}/echo"
    exe_path = pathlib.Path(sys.executable)
    new_bin  = exe_path.with_name("echo_new")

    def _download():
        try:
            root.after(0, lambda: status_var.set("Downloading update…"))
            with _urlopen(bin_url, timeout=120) as resp:
            ...
            # Make executable
            new_bin.chmod(0o755)
            # Shell script: wait for parent to exit, then swap files and relaunch
            sh = exe_path.with_name("_echo_updater.sh")
            sh.write_text(
                "#!/bin/sh\n"
                f'sleep 2\n'
                f'mv -f "{new_bin}" "{exe_path}"\n'
                f'chmod +x "{exe_path}"\n'
                f'"{exe_path}" &\n'
                f'rm -- "$0"\n',
                encoding="utf-8",
...
def _make_tray_image() -> Image.Image:
    icon = _load_icon()
    if icon is not None:
        return icon.resize((85, 85), Image.LANCZOS)
    size = 64
...
def _setup_tray(root: tk.Tk) -> pystray.Icon:
    ...
    icon = pystray.Icon("Echo", _make_tray_image(), "Echo", menu)
    ...
    def _work_area(self) -> tuple[int, int, int, int]:
        ...
        try:
            out = subprocess.check_output(
                ["wmctrl", "-d"], text=True, stderr=subprocess.DEVNULL)
            for line in out.splitlines():
                parts = line.split()
                if "*" in parts:
                    # geometry column: WxH+X+Y
                    for p in parts:
                        m = __import__("re").match(r"(\d+)x(\d+)\+(\d+)\+(\d+)", p)
                        if m:
...
def main():
    ...
    style.configure("Main.TNotebook.Tab",
                    ...
                    padding=[20, 8], font=("Sans Bold", 10))
    ...
    log_box = scrolledtext.ScrolledText(
        ...
        bg=LOG_BG, fg=FG, font=("Monospace", 9),
    ...
    tk.Label(eng_footer, text=f"Config: {user_config.config_path()}",
             bg=BG, fg=MUTED, font=("Sans", 8), anchor="w").pack(side="left")
    ...
    # ── Startup log ────────────────────────────────────────────────────────────
    _log_queue.put(f"Echo v{VERSION} started (Linux)\n")
