...
class _TrainDialog:
    """Overlay dialog that records voice samples for a spoken name."""

    NUM_SAMPLES = 3          # recordings required
    RECORD_SECS = 2.5        # seconds to capture per sample

    def __init__(self, parent: AppManagerWidget, app_name: str,
                 spoken: str, on_close=None):
        self._parent   = parent
        self._app_name = app_name
        self._spoken   = spoken
        self._on_close = on_close
        self._samples: list[bytes | None] = [None] * self.NUM_SAMPLES
        self._recording = False
        self._current   = 0   # which sample slot we're filling

        import voice_templates as vt
        self._vt = vt

        # ── Overlay ──────────────────────────────────────────────────────────
        self._overlay = tk.Frame(parent, bg="#0d0d1a")
        self._overlay.place(x=0, y=0, relwidth=1, relheight=1)
        self._overlay.lift()
        self._overlay.bind("<Escape>", lambda e: self._close())

        card = tk.Frame(self._overlay, bg=CARD, padx=24, pady=20)
        card.place(relx=0.5, rely=0.42, anchor="center")

        tk.Label(card, text=f"🎤  Voice Training — {app_name}",
                 bg=CARD, fg=ACC, font=("Segoe UI Semibold", 12)).pack(pady=(0, 4))
        tk.Label(card,
                 text=f'Say  "{spoken}"  clearly {self.NUM_SAMPLES} times.\n'
                      "Click Record before each attempt.",
                 bg=CARD, fg=FG, font=("Segoe UI", 9), justify="center").pack(pady=(0, 14))

        # Sample indicator dots
        dot_row = tk.Frame(card, bg=CARD)
        dot_row.pack(pady=(0, 16))
        self._dots: list[tk.Label] = []
        for i in range(self.NUM_SAMPLES):
            d = tk.Label(dot_row, text="○", bg=CARD, fg=MUTED,
                         font=("Segoe UI", 18))
            d.pack(side="left", padx=8)
            self._dots.append(d)

        self._status_lbl = tk.Label(card, text=f'Ready — recording slot 1 of {self.NUM_SAMPLES}',
        ...
        self._clear_btn = tk.Button(
            btn_row, text="Clear All", bg=MUTED, fg="#fff",
            activebackground=MUTED, activeforeground="#fff",
        ...
        self._save_btn = tk.Button(
            btn_row, text="💾  Save & Close", bg=GRN, fg="#11111b",
            activebackground=GRN, activeforeground="#11111b",
            relief="flat", font=("Segoe UI Semibold", 9),
            padx=10, pady=6, cursor="hand2", state="disabled",
            command=self._save)
        self._save_btn.pack(side="right")

        tk.Button(btn_row, text="Cancel", bg=MUTED, fg="#fff",
                  activebackground=MUTED, activeforeground="#fff",
                  relief="flat", font=("Segoe UI", 9),
                  padx=10, pady=6, cursor="hand2",
                  command=self._close).pack(side="right", padx=(0, 8))

        self._overlay.after(50, self._overlay.focus_set)

        # Load any existing samples so the user can see prior recordings
        self._reload_existing()

    def _reload_existing(self):
        n = self._vt.template_count(self._spoken)
        for i in range(min(n, self.NUM_SAMPLES)):
            self._samples[i] = b"EXISTING"   # placeholder — already on disk
            self._dots[i].config(text="●", fg=GRN)
        filled = sum(s is not None for s in self._samples)
        if filled > 0:
            self._current = min(filled, self.NUM_SAMPLES - 1)
            self._update_status()
        if filled >= self.NUM_SAMPLES:
    ...
    def _record_sample(self):
        ...
        self._rec_btn.config(state="disabled", text="⏺  Recording…")
        slot = self._current
        ...
        def _run():
            try:
                ...
                n_chunks = int(16000 / 1024 * self.RECORD_SECS)
                for _ in range(n_chunks):
                    buf.append(stream.read(1024, exception_on_overflow=False))
                stream.stop_stream(); stream.close()
            finally:
                pa.terminate()
            self._parent.after(0, lambda: self._finish_record(slot, b"".join(buf)))

        threading.Thread(target=_run, daemon=True).start()

    def _finish_record(self, slot: int, audio: bytes):
        self._recording = False
        self._samples[slot] = audio
        self._dots[slot].config(text="●", fg=GRN)
        ...
        for i in range(self.NUM_SAMPLES):
            ...
            self._current = self.NUM_SAMPLES - 1

        filled = sum(s is not None for s in self._samples)
        self._rec_btn.config(
            state="normal" if filled < self.NUM_SAMPLES else "disabled",
            text="⏺  Record")
        if filled >= self.NUM_SAMPLES:
            self._save_btn.config(state="normal")
        self._update_status()
    ...
    def _update_status(self):
        ...
        if filled >= self.NUM_SAMPLES:
            self._status_lbl.config(
                text=f'All {self.NUM_SAMPLES} samples recorded — click Save & Close.',
                fg=GRN)
        else:
            nxt = self._current + 1
            self._status_lbl.config(
                text=f'Recording slot {nxt} of {self.NUM_SAMPLES} — click Record',
                fg=MUTED)

    def _clear_all(self):
        self._samples = [None] * self.NUM_SAMPLES
        self._current = 0
        for d in self._dots:
            d.config(text="○", fg=MUTED)
        self._rec_btn.config(state="normal", text="⏺  Record")
        self._save_btn.config(state="disabled")
        self._vt.delete_templates(self._spoken)
        self._status_lbl.config(
            text=f'Ready — recording slot 1 of {self.NUM_SAMPLES}', fg=MUTED)

    def _save(self):
        ok = 0
        for i, audio in enumerate(self._samples):
            if audio and audio != b"EXISTING":
                if self._vt.save_template(self._spoken, audio, i):
                    ok += 1
        self._close()
